...
- **Three-phase pipeline.** Phase 1 builds each task group with a fresh Opus implementer, one broad review, and one fix. Phase 2 runs a whole-branch adversarial panel — 3 diverse-lens Opus skeptics plus 1 cross-model Codex reviewer. Phase 3 is an evidence-based QC merge gate that **pushes but never merges**, escalating via `AskUserQuestion` after 3 `NOT_MERGEABLE` rounds.
