...
- **Three-phase pipeline.** Phase 1 builds each task group with a fresh Fable 5 implementer, one broad review, and one fix. Phase 2 runs a whole-branch adversarial panel — 3 diverse-lens Fable 5 skeptics plus 1 cross-model Codex reviewer. Phase 3 is an evidence-based QC merge gate that **pushes but never merges**, escalating via `AskUserQuestion` after 3 `NOT_MERGEABLE` rounds.
