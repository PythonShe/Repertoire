...
- **安装了 OpenAI [`codex-cc` 插件](https://github.com/openai/codex-plugin-cc)的 Codex 账户**——强烈建议。除 Presto 之外，每个 skill 都会通过它调度一个跨模型的 Codex agent（评审者或调查者）。没有 Codex 时 skill 也能优雅降级——它们会改跑纯 Opus 的小组，并在报告中说明这一点——但跨模型校验本身就是设计的一部分。（Presto 刻意不设这个座位：跨模型这一遍的时间成本值得花在整条分支上，而不是一处范围明确的改动上。）
...
| **Presto** | `/repertoire:presto` | 生产线旁的快车道：一处范围明确的改动，一坐之间做完，前面既不需要 spec，也不需要计划。1-2 个只读的 Opus 侦察兵勘定落点并权衡实现路径，用户在唯一一道 AskUserQuestion 闸门上做出选择——闸门通过之前不写一个字。随后由体量闸门决定座位：小改动由控制者亲自实现，省下一次交接的代价；更大的则交给一个实现者 subagent。之后必然跟着一个 Opus 评审者——尤其是在控制者自己就是作者的时候；再走一轮修复（控制者修自己写的以及琐碎的，其余交给修复者 subagent），最后由一道简短的构建 + 测试 QC 闸门收尾。常规路径上最少 3 个 agent，最多 6 个；一次运行若想按设计扩编——第三个侦察兵、第二轮评审、一个多视角小组——就说明它已经超出了快车道。出口有两道：体量过大的工作在审批闸门处转交 Libretto 或 Score，而连续两轮没能通过 QC 的改动则转交 Maestro。这是唯一一个刻意不设跨模型 Codex 座位的 skill——那一遍的时间成本值得花在整条分支上，而不是一处改动上。在匹配的请求上自动触发；未被明确点名时，会先确认简报与分支再进行侦察。 |
