...
| **Presto** | `/repertoire:presto` | 生产线旁的快车道：一处范围明确的改动，一坐之间做完，前面既不需要 spec，也不需要计划。1-2 个只读的 Opus 侦察兵勘定落点并权衡实现路径，用户在唯一一道 AskUserQuestion 闸门上做出选择——闸门通过之前不写一个字。随后由体量闸门决定座位：小改动由控制者亲自实现，省下一次交接的代价；更大的则交给一个实现者 subagent。之后必然跟着一个 Opus 评审者——尤其是在控制者自己就是作者的时候；再走一轮修复（控制者修自己写的以及琐碎的，其余交给修复者 subagent），最后由一道简短的构建 + 测试 QC 闸门收尾。最少 3 个 agent，最多 6 个；一次运行若还想要第 7 个座位，就说明它已经超出了快车道，会在闸门处转交 Maestro。这是唯一一个刻意不设跨模型 Codex 座位的 skill——那一遍的时间成本值得花在整条分支上，而不是一处改动上。在匹配的请求上自动触发；未被明确点名时，会先确认简报与分支再进行侦察。 |
