...
- **Borrowed taste, bundled verbatim.** The operational files — `STANDARDS.md` (review-animations), `AUDIT.md` (improve-animations), and the animation vocabulary — are copied into `skills/legato/references/` with provenance headers, under the upstream MIT license reproduced in `references/LICENSE`. The philosophy documents (emil-design-eng, apple-design, find-animation-opportunities, pick-ui-library) are distilled into the prompt files instead, with the same attribution. The copies are never edited except to re-sync upstream; agents cite them by path and copy values exactly — "never approximate a value that appears in the references" is the skill's load-bearing rule, and it only holds if the references are stable, citable files rather than paraphrase.
