...
運作的 Opus subagent 組成的陣容（實作者、調查員、審查小組、QC 把關），它們的可靠度正是來自於此。
請依此選擇方案：
...
> **模型說明（暫時）：** subagent 目前運行在 **Opus** 上。Anthropic 已於 2026-06-12 依美國出口管制令暫停了 Fable 5 的存取，因此每個席位都回退到 Opus——目前仍可用的最高階模型——並以 xhigh 推理強度運作。一旦 Fable 5 重新上線，我們就會把 subagent 切換回去。
