...
| **Jam** | `/repertoire:jam` | 全自主的即興一場：使用者把鑰匙交出來，Jam 自己找活、自己做完、自己交卷。三位唯讀的 Opus 偵察員並行研讀儲存庫，始終坐在最終使用者的位置上——不破壞相容性的新功能、使用者正默默忍受的快速修正、以及 UI/UX 打磨——接著由一位 Opus 遴選者把候選壓成一份 3-5 個作業、規模恰好一場會話的作業單（docket），向使用者播報即開工，而非等待核可。全新的實作者逐一循序建置作業，是否派出逐作業的 Opus 審查由指揮者裁量（凡觸及行為的必須審查，僅文件與外觀類可以略過）；一個作業兩次受阻就放棄，絕不戀戰。終章從不縮水：兩位整分支審查員並行——一位固定用 Opus，一位跑在會話模型上，正是這對跨檔搭檔頂替了這場無人值守執行刻意不設的 Codex 席位——接著是一道以證據為本的 QC 關卡，會建置、測試，並能用乾淨的 revert 截除無法交付的作業。QC 兩振出局即升級給使用者；只在被要求時推送，絕不合併。會在符合的請求上自動觸發；未被明確點名時，會先確認範圍與成本再進行調度。 |
