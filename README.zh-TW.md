...
- **安裝了 OpenAI [`codex-cc` 外掛](https://github.com/openai/codex-plugin-cc)
  的 Codex 帳號** —— 強烈建議。除 Presto 之外，每個 skill 都會透過它調度一個跨模型的 Codex agent
  （審查員或調查員）。沒有 Codex 時 skill 仍能優雅降級——它們會改跑純 Opus 的小組，
  並在報告中說明——但跨模型檢查本來就是設計的一部分。（Presto 刻意不設這個席位：
  跨模型這一輪的時間成本值得花在整條分支上，而不是一處範圍明確的變更上。）
...
| **Presto** | `/repertoire:presto` | 生產線旁的快車道：一處範圍明確的變更，一次坐定就完成，前面既不需要 spec，也不需要計畫。1-2 位唯讀的 Opus 偵察員勘定落點並權衡實作路徑，使用者在唯一一道 AskUserQuestion 關卡上做出選擇——關卡通過之前不寫下任何一個字。接著由規模關卡決定座位：小變更由控制者親自實作，省下一次交接的代價；更大的則交給一個實作者 subagent。之後必定跟著一位 Opus 審查員——尤其是在控制者自己就是作者的時候；再走一輪修正（控制者修自己寫的以及瑣碎的，其餘交給修正者 subagent），最後以一道簡短的建置 + 測試 QC 關卡收尾。常規路徑上最少 3 個 agent，最多 6 個；一次執行若想按設計擴編——第三位偵察員、第二輪審查、一個多視角小組——就代表它已經超出快車道。出口有兩道：規模過大的工作在核可關卡處轉交 Libretto 或 Score，而連續兩輪未能通過 QC 的變更則轉交 Maestro。這是唯一一個刻意不設跨模型 Codex 座位的 skill——那一輪的時間成本值得花在整條分支上，而不是一處變更上。會在符合的請求上自動觸發；未被明確點名時，會先確認簡報與分支再進行偵察。 |
