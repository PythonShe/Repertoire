...
when_to_use: Use when the user reports a reproducible bug, regression, crash, or failing test in the codebase and wants it diagnosed to root cause and fixed — "fix this bug", "debug this failure", "why is this test failing", "find the root cause", "run Tuner on this". Code that works but should be better goes to encore; an animation that feels wrong rather than broken goes to legato.
---
