...
- **No cross-model seat, by design.** Presto seats no Codex reviewer — a
  variance it shares with legato (same reason) and jam (its own reason;
  `shared/invariants.md` records all three). The cross-model pass is worth its wall-clock on a
